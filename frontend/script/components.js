const Componentes = {
    ...
    return `
      <div class="panel checkout-panel" id="checkoutPanel" aria-hidden="true">
        ...
        <div class="panel-footer">
          ...
          <small style="display:block;text-align:center;margin-top:8px;color:#666">Após confirmar, o pagamento abre em uma janela centralizada</small>
        </div>
      </div>
    `;
  },
  ...
  async abrirCheckout() {
    ...
    // Fecha a sacola antes de abrir o checkout (evita os dois painéis sobrepostos)
    this.fecharSacola();
    ...
    // O Brick abre em modal centralizada após Confirmar Pedido (finalizarCompra)
  },

  // Modal centralizada de pagamento (responsiva; evita o Brick espremido no painel lateral)
  renderizarModalPagamento() {
    return `
      <div class="modal-overlay pagamento-overlay" id="pagamentoModalOverlay" onclick="Componentes.fecharModalPagamento(event)">
        <div class="modal pagamento-modal" onclick="event.stopPropagation()">
          <div class="modal-body pagamento-modal-body">
            <button class="panel-close pagamento-close" onclick="Componentes.fecharModalPagamento()" aria-label="Fechar">&times;</button>
            <h2 class="modal-title">Pagamento</h2>
            <p class="modal-text" id="pagamentoResumo"></p>
            <div id="brick_container_modal"></div>
          </div>
        </div>
      </div>
    `;
  ...
  abrirModalPagamento({ idPedido, valorTotal, entrega_tipo }) {
    // Guarda o valor do pedido criado (a sacola já foi limpa; o Brick NÃO pode ler do carrinho)
    this._pagamentoValor = valorTotal;
    let overlay = document.getElementById('pagamentoModalOverlay');
    if (!overlay) {
      const holder = document.createElement('div');
      holder.innerHTML = this.renderizarModalPagamento();
      overlay = holder.firstElementChild;
      document.body.appendChild(overlay);
    }
    const resumo = document.getElementById('pagamentoResumo');
    if (resumo) resumo.innerHTML = `Pedido <strong>${idPedido}</strong> • Total <strong>${api.formatarMoeda(valorTotal)}</strong>${entrega_tipo ? ` • ${entrega_tipo}` : ''}`;
    overlay.classList.add('ativo');
    document.body.classList.add('modal-open');
    setTimeout(() => this._initBrick('brick_container_modal'), 300);
  },

  // Traduz status_detail do MP para mensagem clara (cartão recusado)
  traduzirRecusa(detail) {
    const mapa = {
  ...
  fecharModalPagamento(e) {
    if (e && e.target !== e.currentTarget && e.target.tagName !== 'BUTTON') return;
    const overlay = document.getElementById('pagamentoModalOverlay');
    if (overlay) overlay.classList.remove('ativo');
    // Não remove modal-open se outra modal (sucesso) estiver ativa
    if (!document.getElementById('modalSucesso')?.classList.contains('ativo')) {
      document.body.classList.remove('modal-open');
    }
  },

  async _initBrick(containerId = 'brick_container_modal') {
    if (this._brickInitializing) return;      // trava contra chamadas concorrentes
    this._brickInitializing = true;
    try {
      // Valor vem do pedido criado (sacola já limpa) — carrinho só como fallback
      let valor = this._pagamentoValor;
      if (!valor || valor <= 0) {
        try { valor = JSON.parse(localStorage.getItem('ultimo_pedido') || '{}').valor_total; } catch (_) {}
      }
      if (!valor || valor <= 0) {
        valor = this._freteValor ? (Carrinho.subtotal() + this._freteValor) : Carrinho.subtotal();
      }
      if (!valor || valor <= 0) {
        const contV = document.getElementById(containerId);
        if (contV) contV.innerHTML = '<div class="brick-loading">Não foi possível determinar o valor do pedido. Feche e confirme o pedido novamente.</div>';
        return;
      }
      const user = api.getUser(); const email = (user?.username && user.username.includes('@')) ? user.username : 'comprador@email.com';
      // Public Key vem 100% do .env via backend (GET /webhook/public-key).
      // Sem fallbacks locais: chave errada aqui = pagamento real recusado pelo MP.
      const pkData = await api.request('/webhook/public-key').catch(() => ({ public_key: '' }));
      const pk = (pkData.public_key || '').trim();
      if (!pk || !/^(TEST|APP_USR)-/.test(pk)) {
        console.warn('Brick: Public Key ausente/inválida no .env (MERCADOPAGO_PUBLIC_KEY)');
        const cont0 = document.getElementById(containerId);
        if (cont0) cont0.innerHTML = '<div class="brick-loading">Pagamento indisponível no momento. Tente novamente mais tarde.</div>';
        this.toast('Pagamento indisponível: Public Key não configurada no servidor.', 'error');
        return;
      }
      ...
      if (!window.MercadoPago) {
        try {
          await new Promise((res, rej) => {
            ...
            s.onload = res; s.onerror = () => rej(new Error('SDK bloqueado'));
            document.head.appendChild(s);
          });
        } catch (_) {
          throw new Error('SDK do Mercado Pago bloqueado — desative AdBlock/proteção anti-rastreamento para este site e tente de novo');
        }
      }
      ...
      // ORDEM CORRETA: desmonta primeiro, deixa o SDK limpar o próprio DOM
      if (this._brickCtrl) {
        try { await this._brickCtrl.unmount(); } catch (_) { }
        this._brickCtrl = null;
      }
      // Loading visível enquanto o Brick do MP monta os iframes
      cont.innerHTML = '<div class="brick-loading"><span class="brick-spinner"></span>Carregando opções de pagamento...</div>';

      // Sem preferenceId (pagamento direto via /v1/payments): não envia mercadoPago
      const criarBrick = bricks.create('payment', containerId, {
        ...
        callbacks: {
          ...
          onSubmit: async ({ selectedPaymentMethod, formData }) => {
            console.log('[Brick onSubmit]', selectedPaymentMethod, formData);
            const raw = localStorage.getItem('ultimo_pedido');
            const pedidoId = raw ? JSON.parse(raw).id : null;
            if (!pedidoId) { this.toast('Crie o pedido em Confirmar Pedido antes de pagar', 'error'); return new Promise(() => {}); }
            const idempotencyKey = `pedido-${pedidoId}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
            try {
              // formData.payment_method_id é o id real (ex.: visa/master para débito);
              // selectedPaymentMethod pode ser o grupo genérico (debit_card) que o MP rejeita
              const metodo = formData.payment_method_id || selectedPaymentMethod;
              // Preserva dados do portador coletados pelo Brick (nome/identificação) + garante email
              const payerBrick = { ...((formData && formData.payer) || {}), email: ((formData && formData.payer) || {}).email || email };
              const res = await api.request('/api/payments/process', { method: 'POST', headers: { 'X-Idempotency-Key': idempotencyKey }, body: JSON.stringify({ pedido_id: pedidoId, formData: { ...formData, payment_method_id: metodo, payer: payerBrick } }) });
              if (res.qr_code_base64) {
                this.mostrarPixQR(res, { id: pedidoId, valor_total: valor });
                return new Promise(() => {});
              }
              if (['approved', 'authorized'].includes((res.status || '').toLowerCase())) {
                Carrinho.limpar(); this.atualizarContador();
                this.fecharModalPagamento();
                await this.mostrarModalSucesso({ id: pedidoId, valor_total: valor, entrega_tipo: this._entregaTipo });
              } else if ((res.status || '').toLowerCase() === 'in_process' || (res.status || '').toLowerCase() === 'pending') {
                this.toast('Pagamento em análise. A confirmação chega sozinha aqui.', 'info');
                return new Promise(() => {});
              } else {
                // Recusado/cancelado: explica o motivo e reabre o form limpo (nunca trava no loading)
                this.toast(this.traduzirRecusa(res.status_detail), 'error');
                try { if (this._brickCtrl) { await this._brickCtrl.unmount(); this._brickCtrl = null; } } catch (_) {}
                this._brickInitializing = false;
                await this._initBrick('brick_container_modal');
              }
            } catch (e) { this.toast(e.message, 'error'); }
            // Promise pendente: o Brick não assume a tela de resultado; nosso modal/polling conduz
            return new Promise(() => {});
          },
          onError: (e) => {
            console.error('brick error', e);
            this.toast('Erro ao carregar o pagamento. Tente novamente.', 'error'); // <-- feedback visível
          }
        }
      });
      // Timeout: se o Brick não montar em 15s, mostra erro + botão de tentar de novo (nunca trava no loading)
      const timeoutBrick = new Promise((_, rej) => setTimeout(() => rej(new Error('Tempo esgotado ao carregar o pagamento (15s)')), 15000));
      this._brickCtrl = await Promise.race([criarBrick, timeoutBrick]);
    } catch (e) {
      console.warn('Brick init falhou', e);
      this._brickCtrl = null;
      const cont2 = document.getElementById(containerId);
      if (cont2) cont2.innerHTML = `<div class="brick-loading" style="flex-direction:column;gap:12px">Não foi possível carregar o pagamento (${e.message || e}).<button class="btn btn-primary btn-sm" onclick="Componentes._brickInitializing=false;Componentes._initBrick('brick_container_modal')">Tentar novamente</button></div>`;
      this.toast('Não foi possível carregar as opções de pagamento.', 'error'); // <-- feedback visível
    } finally {
      this._brickInitializing = false;
    }
  },
  ...
  voltarSacola() {
    // Fecha o checkout e reabre a sacola pelo fluxo único (atualiza overlay e _currentPanel)
    this.renderizarItensSacola();
    this._openPanel('sacolaPanel');
  },
  ...
  async finalizarCompra(e) {
    e.preventDefault();
    if (this._finalizando) return; // trava duplo-tap no Confirmar Pedido
    ...
    try {
      ...
      // Fecha sacola + checkout e abre a modal centralizada de pagamento (Brick)
      ...
      Carrinho.limpar();
      this.atualizarContador();
      this.abrirModalPagamento({ idPedido: pedido.id_pedido || '#' + pedido.id, valorTotal, entrega_tipo });
  ...
  // QR Pix renderizado DENTRO da modal de pagamento (o onSubmit suprime a tela do Brick)
  mostrarPixQR(pix, pedido) {
    const idp = pedido.id_pedido || '#' + pedido.id;
    const cont = document.getElementById('brick_container_modal');
    if (cont) {
      cont.innerHTML = `
        <div style="text-align:center">
          <p>Escaneie o QR Code PIX para pagar <strong>${api.formatarMoeda(pix.valor || pedido.valor_total)}</strong> • Pedido <strong>${idp}</strong></p>
          ...
          <div style="display:flex;gap:8px;margin-top:10px">
            <input id="pixCopiaCola" readonly value="${pix.qr_code || ''}" style="flex:1;min-width:0;font-size:.75rem;padding:8px;border:1px solid #ddd;border-radius:8px;background:#f8f9fa" onclick="this.select()">
            <button class="btn btn-primary btn-sm" onclick="navigator.clipboard.writeText(document.getElementById('pixCopiaCola').value);Componentes.toast('Código Pix copiado!','success')">Copiar</button>
          </div>
          <p style="font-size:.82rem;color:#137333;margin-top:10px"><span class="brick-spinner" style="display:inline-block;vertical-align:-4px;margin-right:6px"></span>Aguardando pagamento... a confirmação abre sozinha aqui.</p>
        </div>`;
    }
    // poll de backup (o verificarRetornoPagamento global também roda via ultimo_pedido)
    let tent = 0; const iv = setInterval(async () => {
      ...
      if (atualizado && ['pago', 'enviado', 'entregue'].includes((atualizado.status || '').toLowerCase())) {
        clearInterval(iv);
        try {
          const raw = localStorage.getItem('ultimo_pedido');
          const lp = raw ? JSON.parse(raw) : {};
          atualizado.entrega_tipo = atualizado.entrega_tipo || lp.entrega_tipo;
        } catch (_) {}
        this.fecharModalPagamento();
        this.mostrarModalSucesso(atualizado);
        localStorage.removeItem('ultimo_pedido');
